<?php $post_data = base64_decode($infouser);
  ...
  <div class="chat-head">
      ...
      <?php echo form_open(base_url() . 'teacher/message/send_new/', array('class' => 'form', 'enctype' => 'multipart/form-data')); ?>
					<select class="form-control rounded" name="reciever" required>
						<option value=""><?php echo get_phrase('reciever');?></option>
            ...
            <optgroup label="<?php echo get_phrase('teachers');?>">
                <?php
                    ...
                    <?php if($this->session->userdata('login_user_id') != $row['teacher_id']):?>
                      <option value="teacher-<?php echo $row['teacher_id']; ?>" <?php if($usertype == 'teacher' && $user_id == $row['teacher_id']) echo 'selected';?>><?php echo $row['name']; ?></option>
                    <?php endif;?>
                <?php endforeach; ?>
            </optgroup>
