...
<ul class="breadcrumb hidden-xs-down hidden-sm-down">
  ...
  <?php $profile_info = $this->db->get_where('teacher' , array('teacher_id' => $this->session->userdata('login_user_id')))->result_array();
      ...
      <img alt="" src="<?php echo $this->crud_model->get_image_url('teacher', $row['teacher_id']);?>">
          ...
          <h6 class="up-sub-header">
            <?php echo get_phrase('account_type');?>: 
               <div class="value badge badge-pill badge-primary"><?php echo get_phrase('teacher');?></div>
          </h6>
        </div>
    ...
    <?php $photo = $this->db->get_where('teacher', array('teacher_id' => $this->session->userdata('login_user_id')))->row()->fb_photo;?>
    <?php $name = $this->db->get_where('teacher', array('teacher_id' => $this->session->userdata('login_user_id')))->row()->fb_name;?>
    <?php $id = $this->db->get_where('teacher', array('teacher_id' => $this->session->userdata('login_user_id')))->row()->fb_id;?>
    <?php $salir = $this->db->get_where('teacher', array('teacher_id' => $this->session->userdata('login_user_id')))->row()->logoutURL;?>
    <?php $gid = $this->db->get_where('teacher', array('teacher_id' => $this->session->userdata('login_user_id')))->row()->g_oauth;?>
    <?php $fname = $this->db->get_where('teacher', array('teacher_id' => $this->session->userdata('login_user_id')))->row()->g_fname;?>
    <?php $lname = $this->db->get_where('teacher', array('teacher_id' => $this->session->userdata('login_user_id')))->row()->g_lname;?>
    <?php $gphoto = $this->db->get_where('teacher', array('teacher_id' => $this->session->userdata('login_user_id')))->row()->g_picture;?>
    <?php $gemail = $this->db->get_where('teacher', array('teacher_id' => $this->session->userdata('login_user_id')))->row()->g_email;?>
    ...
    <div class="pricing-plans row no-gutters">
      <div class="pricing-plan col-sm-6">
        ...
        <div class="plan-body"><br><br>
          <div class="plan-btn-w">
            ...
            <?php else:?>
              <a class="btn btn-danger btn-rounded" href="<?php echo base_url();?>teacher/my_account/remove_facebook/">Unlink</a>
            <?php endif;?>
          </div>
        </div>
      </div>
      <div class="pricing-plan col-sm-6">
        ...
        <div class="plan-body"><br><br>
          <div class="plan-btn-w">
            ...
            <?php else:?>
              <a class="btn btn-danger btn-rounded" href="<?php echo base_url();?>teacher/my_account/remove_google/">Unlink</a>
            <?php endif;?>
          </div>
        </div>
      </div>
      ...
      foreach ($subjects as $subject): ?>
        <tr>
          ...
          <td style="text-align: center;"><a class="btn nc btn-rounded btn-sm btn-primary" style="color:white"><?php echo $this->db->get_where('class', array('class_id' => $subject['class_id']))->row()->name;?></a></td>
          <td style="text-align: center;"><a class="btn nc btn-rounded btn-sm btn-purple" style="color:white"><?php echo $this->db->get_where('section', array('class_id' => $subject['class_id']))->row()->name;?></a></td>
          <td style="text-align: center;"><a class="btn nc btn-rounded btn-sm btn-success" style="color:white"><?php $this->db->where('class_id', $subject['class_id']); echo $this->db->count_all_results('enroll');?></a></td>
        </tr>
      <?php endforeach;?>
      </tbody>
      </table>
...
<?php 
    $edit_data = $this->db->get_where('teacher' , array('teacher_id' => $this->session->userdata('login_user_id')))->result_array();
    foreach ($edit_data as $row3):
?>
  <div class="col-sm-7">
    <div class="element-wrapper">
      <div class="element-box">
        <?php echo form_open(base_url() . 'teacher/my_account/update/'.$this->session->userdata('login_user_id'), array('id' => 'formValidate', 'enctype' => 'multipart/form-data'));?>
