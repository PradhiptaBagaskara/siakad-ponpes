...
<ul class="breadcrumb hidden-xs-down hidden-sm-down">
	<div class="back">
		<a href="<?php echo base_url();?>student/forum/"><i class="os-icon os-icon-common-07"></i></a>
	</div>
</ul>
  <div class="content-i">
	...
	<div class="col-sm-8">
		...
		<div class="element-box shadow lined-success">
			...
			<input type="hidden" value="<?php echo $post_code;?>" name="post_code" id="post_code">
	...
	<div id="panel">
		...
		<div class="element-box-w b-t">
            <div class="row m-t m-b">
			  <div class="col-sm-10">
				  <a href="#"><img alt="" src="<?php echo $this->crud_model->get_image_url($row2['user_type'], $row2['user_id']); ?>" width="30px" style="border-radius:20px;margin-right:5px;"> <span class="infogi"><?php echo $this->db->get_where($row2['user_type'] , array($row2['user_type'] . '_id' => $row2['user_id']))->row()->name;?></span></a>
...
<script>
	...
	$(document).ready(function()
	{
	  $("#add").click(function()
	  {
	  	...
	  	message= $("#message").val();
	  	if(post_code!="" && message!="")
	  	{
		  	$.ajax({url:"<?php echo base_url();?>student/forum_message/add/",type:'POST',data:{message:message,post_code:post_code},success:function(result)
		  	{
