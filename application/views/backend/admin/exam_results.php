	  ...
	  <div class="os-tabs-w menu-shad">
		<div class="os-tabs-controls">
		  <ul class="nav nav-tabs upper">
			...
			<li class="nav-item">
			  <a class="nav-link active" href="<?php echo base_url();?>admin/exam_results/<?php echo $exam_code;?>/"><i class="os-icon picons-thin-icon-thin-0100_to_do_list_reminder_done"></i><span><?php echo get_phrase('results');?></span></a>
			</li>
			<li class="nav-item">
		  ...
		  <div class="pipeline-header">
			...
			<div class="table-responsive">
			 <?php $students   =   $this->db->get_where('enroll' , array('class_id' => $row['class_id'], 'section_id' => $row['section_id'] , 'year' => $running_year))->result_array();
                foreach($students as $rows1):?>
                <?php  
                    $count = 1;
                    $this->db->where('student_id',$rows1['student_id']);
                    ...
                    $questions = $this->db->get('student_question')->result_array();
                            foreach($questions as $row2): ?>
                            	...
                            	<?php 
                                	...
                                	<?php if($ans[$i] == $row3['correctanswer']) {$mark+=$row3['marks']; $anscor+=1;}?>
                  		...
                  		<tbody>	
                    		<tr>
                      			<td style="min-width:170px">
                        			<img alt="" src="<?php echo $this->crud_model->get_image_url('student', $rows1['student_id']); ?>" width="25px" style="border-radius: 10px;margin-right:5px;"> <?php echo $this->db->get_where('student' , array('student_id' => $rows1['student_id']))->row()->name; ?>
                      			</td>
